package ce3.tutorial.concurrentqueue
...
final private case class State[F[_], A](
    queue: ScQueue[A],
    capacity: Int,
    takers: ScQueue[Deferred[F, A]],
    offerers: ScQueue[(A, Deferred[F, Unit])]
)

private object State {
  def empty[F[_], A](capacity: Int): State[F, A] =
    State(ScQueue.empty[A], capacity, ScQueue.empty[Deferred[F, A]], ScQueue.empty[(A, Deferred[F, Unit])])
}
